...
import { categories, getProducts, getSiteConfig, getWhatsAppOrderLink, type Product, type ProductCategory } from "../data";
...
export function Home() {
  ...
  const heroFrames = products.length > 0
    ...
    : [{
        ...
        description: config.heroSubtitle || config.tagline || "Katalog polaroid yang bergerak mengikuti scroll.",
        image: "/assets/catalog-home-rp150000-item-02.jpg",
        images: ["/assets/catalog-home-rp150000-item-02.jpg"],
        price: "",
        priceLabel: "Katalog",
        tag: "hero",
        variant: "hero",
  ...
  return (
    <PageTransition>
          ...
          <div className="mx-auto grid w-full max-w-7xl items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.15 }} style={{ maxWidth: "620px" }}>
              ...
              <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(22px, 3.5vw, 38px)", fontWeight: 400, fontStyle: "italic", color: "#F9F9F7", margin: "0 0 18px 0", letterSpacing: "0.05em", opacity: 0.95 }}>
                Rangkaian segar, snack unik, money bouquet eksklusif
              </p>
      ...
      <div style={{ padding: "0 clamp(24px, 8vw, 120px)", textAlign: "center" }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }} style={{ marginBottom: "60px", paddingBottom: "60px" }}>
          <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(28px, 5vw, 48px)", fontWeight: 400, color: "#1a1a1a", marginBottom: "20px", letterSpacing: "0.02em" }}>
            Setiap Polaroid Menceritakan Kisah
          </p>
          <p style={{ maxWidth: "680px", margin: "0 auto", color: "#555", fontFamily: "'Inter', sans-serif", fontSize: "16px", lineHeight: 1.8 }}>
            Dari buket satin yang elegan hingga money bouquet yang eksklusif, setiap koleksi kami dirancang khusus untuk membuat moment Anda lebih berkesan dan bermakna.
          </p>
        </motion.div>
      </div>
...
function ProductDetailModal({ product, onClose }: { product: Product; onClose: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: "fixed", inset: 0, zIndex: 9999, backgroundColor: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "clamp(16px, 4vw, 40px)" }} onClick={onClose}>
      <motion.div initial={{ y: 40, scale: 0.95 }} animate={{ y: 0, scale: 1 }} exit={{ y: 20, scale: 0.95 }} transition={{ type: "spring", damping: 25, stiffness: 300 }} onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: "900px", backgroundColor: "#F9F9F7", display: "flex", flexDirection: "row", flexWrap: "wrap", maxHeight: "90vh", overflowY: "auto", position: "relative", boxShadow: "0 24px 60px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ flex: "1 1 320px", padding: "28px", display: "flex", flexDirection: "column", gap: "14px", justifyContent: "center" }}>
          <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "9px", color: "#999", textTransform: "uppercase", margin: 0 }}>{product.category}</p>
          <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(32px, 4vw, 50px)", lineHeight: 1, margin: 0, color: "#1a1a1a" }}>{product.name}</h2>
          <p style={{ fontFamily: "'Inter', sans-serif", color: "#555", lineHeight: 1.8, margin: 0 }}>{product.description}</p>
          <p style={{ fontFamily: "'Inter', sans-serif", fontWeight: 600, color: "#1a1a1a", margin: 0 }}>{product.price}</p>
          <a href={getWhatsAppOrderLink(product)} target="_blank" rel="noopener noreferrer" style={{ display: "inline-block", marginTop: "8px", padding: "12px 18px", backgroundColor: "#1a1a1a", color: "#fff", textDecoration: "none", textAlign: "center", borderRadius: "8px", fontFamily: "'JetBrains Mono', monospace", fontSize: "11px", letterSpacing: "0.08em", textTransform: "uppercase" }}>Pesan via WhatsApp</a>
        </div>
      </motion.div>
    </motion.div>
  );
